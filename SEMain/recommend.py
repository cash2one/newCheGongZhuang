...
from hint_top import getRemoteClient,getRemoteClient_2
reload(sys)
        ...
        for i in xrange(0,len(titleDiv)):
            ...
            for j in range(0,len(items)):
                try:
                    ...
                    print "get baidu recommend failed"
            mgroup['data']=mdatas
...
def so360Recommend(word,flag=False):
    ...
    '''try:
        ...
        print "get recommend from 360so failed"
        return''' 
    result=getRemoteClient_2(url)
    #('ISO-8859-1')
    print result[1]
    result = result[0].encode(result[1]).decode('utf-8')
    # print result
    so360Extract(result)
