...
import re
import threading
from hint_top import getRemoteClientHeader,getRemoteClient,getRemoteClientHeader_2
...
def getBaiduRelateDict(word):
   ...
   # print '正在获取百度相关搜索...'
    ...
    print "baidustart :",st,end,end-st   
...
def getChinaRelateDict(word):
    ...
    '''try:
        ...
        print format_exc()
        return None '''
    mingtemp=getRemoteClientHeader(url,header)
    tree=etree.HTML(mingtemp)   
...
def gets360RelateDict(word): 
    ...
    tree = etree.HTML(result[0].encode(result[1]).decode('utf-8')) 
...
def getBingRelateDict(word):
    ...
    '''try:
        ...
        print format_exc()
        return None'''
    result=getRemoteClientHeader(url,header)
    tree = etree.HTML(result)
